...
#bayesmark-launch -dir $DB_ROOT -b $DBID -n $N_STEP -r $N_REPEAT -p $N_BATCH -o pysot hyperopt nevergrad turbo random-search --opt-root $OPT_ROOT -v -c lasso -d breast

bayesmark-launch -dir $DB_ROOT -b $DBID -n $N_STEP -r $N_REPEAT -p $N_BATCH -o hyperopt pysot turbo pyopt tuopt tusot --opt-root $OPT_ROOT -v -d breast
